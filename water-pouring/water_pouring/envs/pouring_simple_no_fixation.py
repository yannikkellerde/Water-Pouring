...
class Pouring_no_fix(Pouring_base):
    def __init__(self,use_gui=False,uncertainty=0):
        super(Pouring_no_fix, self).__init__(use_gui=use_gui,uncertainty=uncertainty,scene_file=os.path.join(FILE_PATH,"scenes","simple_scene.json"))
        self.action_space = spaces.Box(low=0,high=1,shape=(1,))
        self.observation_space = spaces.Box(low=0,high=1,shape=(4,))
    def step(self,action):
        res = super(Pouring_no_fix,self).step((action[0],0.5,0.5,1))
