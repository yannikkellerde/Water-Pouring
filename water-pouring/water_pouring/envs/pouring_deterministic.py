...
class Pouring_deterministic(gym.Env):
    metadata = {'render.modes': ['human']}
    def __init__(self,use_gui=False):
        self.base = pysplishsplash.Exec.SimulatorBase()
        self.base.init(useGui=use_gui,outputDir=os.path.join(FILE_PATH,"particles"),sceneFile=os.path.join(FILE_PATH,"scenes/base_scene.json"))
        ...
        self.max_rotation_radians = 0.001
        self.max_translation_x = 0.004
        self.max_translation_y = 0.004
    ...
    def _get_particle_locations(self):
        ...
        fluid_positions = np.array(self.fluid.getAllPositions()[:self.fluid.numActiveParticles()])
    ...
    def step(self,action):
        ...
        if len(action)!=4 or not np.all(action<=1) or not np.all(action>=0) or action[3] not in (0,1):
